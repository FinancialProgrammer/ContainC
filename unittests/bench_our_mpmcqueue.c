#define _POSIX_C_SOURCE 200112L // for pthread_barrier_t (https://stackoverflow.com/questions/61647896/unknown-type-name-pthread-barrier-t)
...
void* producer_thread(void* arg) {
  ...
  while (atomic_load(&ctx->running) && produced < items_per_producer) {
    ...
    success = queue_put(ctx->queue, data);
...
void* consumer_thread(void* arg) {
  ...
  while (atomic_load(&ctx->running) || 
    ...
    data = queue_get(ctx->queue, 1);  // 1ms timeout
    success = (data != NULL);
...
int main(int argc, char* argv[]) {
  ...
  ctx.queue = queue_create(0, NULL);  // Unlimited size, default allocator
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
...
/*
  ...
  ctx.queue = queue_create(0, &pool_allocator);  // Unlimited size with pool allocator
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
  ...
  ctx.queue = queue_create(0, &item_pool);
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
  ...
  ctx.queue = queue_create(0, &tls_allocator);
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
  ...
  ctx.queue = queue_create(0, &tls_pool_allocator);
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
  ...
  ctx.queue = queue_create(0, &aligned_allocator);
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
  ...
  ctx.queue = queue_create(0, &aligned_pool);
  queue_set_buffer_size(ctx.queue, ctx.buffer_size);
